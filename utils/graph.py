...
def print_dataset_summary(name,data):

    num_samples = len(data)
    num_nodes = len(data.x) if hasattr(data, "x") and data.x is not None else None
    num_classes = len(torch.unique(data.y)) if hasattr(data, "label") and data.y is not None else None
    num_edges = data.edge_index.shape[1] if hasattr(data, "edge_index") and data.edge_index is not None else None
    print(f"\n=== 数据集 {name} 的基本信息 ===")
    print(f"数据集样本数 (num_samples): {num_samples}")
    print(f"每个图的节点数 (num_nodes): {num_nodes}")
    print(f"类别数 (num_classes): {num_classes}")
    print(f"边数 (num_edges): {num_edges}")

def pad_sequence(token, max_length=500):
        """填充序列到 max_length"""
        sequence_length = token.size(1)
        padded = torch.zeros((1, max_length), dtype=torch.long)
        padded[:, :min(sequence_length, max_length)] = token[:, :max_length]
        return padded.squeeze(0)

def process_batch(feature):
    num_nodes = len(feature)
    input_ids = torch.stack(
                    [pad_sequence(feature[i]["input_ids"]) for i in range(num_nodes)]
                )
    attention_mask = torch.stack(
                    [pad_sequence(feature[i]["attention_mask"]) for i in range(num_nodes)]
                )
    masked_input_ids = torch.stack(
                    [pad_sequence(feature[i]["masked_input_ids"]) for i in range(num_nodes)]
                )
...
def edge_dropout(edge_index, drop_prob, training=True):
    if not training or drop_prob == 0.0:
        return edge_index

    num_edges = edge_index.size(1)
    keep_prob = 1 - drop_prob
    sample_mask = torch.rand(num_edges, device=edge_index.device) < keep_prob

    edge_index_kept = edge_index[:, sample_mask]
