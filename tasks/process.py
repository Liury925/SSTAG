...
def text2feature(
        ...
        encoder: Any):
    """
    Encode a collection of strings into tokenized representations.

    Args:
        texts (Union[list[Any], np.ndarray]): Collection of texts (list or np.ndarray).
        encoder (Any): Any module that implements `encode_text` to tokenize text.

    Returns:
        List of dictionaries containing `input_ids` and `masked_input_ids`.
    """
    if isinstance(texts[0], str):
        if isinstance(texts, np.ndarray):
            text_tokens = encoder.encode_text(texts.tolist())  # 返回包含 masked_input_ids 的 tokenized 结果
        else:
            text_tokens = encoder.encode_text(texts)

        return [
            {key: val for key, val in batch.items()}  # 确保所有字段都转换为 Python 列表
            for batch in text_tokens
        ]
...
def feature_embedding_process(
    ...
    if from_saved and file_name is not None:
        tokens = torch_safe_load(file_name)
        if tokens is None:
            if encoder is None:
                raise ValueError("There is no saved tokenized data. Please provide an encoder or check the saved file.")
        else:
            return tokens

    tokens = text2feature(texts, encoder)
    if file_name is not None:
        torch_safe_save(tokens, file_name)
    return tokens
