...
from datasets_process.data import TAGDataset, TAGData
from utils.graph import edge_index_to_sparse_csr, personalized_pagerank, process_batch
...
class BaseTask(Dataset, ABC):
    """Base class for generate tasks supporting multiple splits."""

    def __init__(self, dataset: TAGDataset, split: str = "train",
                 save_data: bool = False, from_saved: bool = True,
                 save_name: Optional[str] = None,
                 post_funcs: Optional[Union[Callable, list[Callable]]] = None,
                 filter_func: Optional[Callable] = None, **kwargs) -> None:
        super().__init__()
        self.dataset = dataset
        self.split = split
        self.save_data = save_data
        self.data = dataset._data
        self.save_name = save_name or self.default_save_name
        self.base_collater = Collater(None, None)
        self.post_funcs = post_funcs if isinstance(post_funcs, list) else ([post_funcs] if post_funcs else [])
        self.filter_func = filter_func
        self.ppr_scores = None
        self.root = osp.join(dataset.root, getattr(dataset, "sub_name", ""), "task")
        self.dataset_name = getattr(dataset, "sub_name", dataset.name)

        print(f"Start building {self.__class__.__name__} on dataset {self.dataset_name}...")

        if from_saved and self.from_saved():
            return
        self.sample_indexs = self.sample_labels = self.sample_label_map = None
        self.node_features = self.edge_features = self.label_features = None
        self.question_features = self.answer_features = self.additional_data = None
        self.data_list = []
        ...
        self.data = self.sample_indexs = self.sample_labels = self.sample_label_map = None
    ...
    def __getitem__(self, item: int) -> Any:
        ...
        if isinstance(self.node_features, torch.Tensor):
            ...
            data.x = [self.node_features[i] for i in node_map.tolist()]
        if isinstance(self.label_features, torch.Tensor):
            data.label = self.label_features[label_map.numpy()]
        else:
            data.label = [self.label_features[i] for i in label_map.tolist()]
        if hasattr(data,"ppr") is not None:
            if len(node_map)<len(data.ppr):
                data.ppr = data.ppr[node_map]
        if self.edge_features is not None:
            data.edge_attr = [self.edge_features[i] for i in edge_map.tolist()]
    ...
    def batch_unique_feature(self, features: Union[Tensor, np.ndarray, list]):
        if isinstance(features, Tensor):
            ...
            feature_map = torch.arange(features.size(0), dtype=torch.long, device=features.device)
            ...
            if isinstance(features[0], list) and isinstance(features[0][0], dict):
                features = np.concatenate(features, axis=0)
                unique_feature = process_batch(features)
                feature_map = torch.arange(len(features), dtype=torch.long)

            elif isinstance(features[0], np.ndarray):
                features = np.concatenate(features, axis=0)
                unique_feature, feature_map = np.unique(features, return_inverse=True, axis=0)
                feature_map = torch.from_numpy(feature_map).long()

            elif isinstance(features[0], list) and isinstance(features[0][0], str):
                ...
                raise ValueError(f"Unsupported feature format: {type(features[0])}")
        else:
...
class DefaultTask(BaseTask):
    ...
    def __build_sample__(
        ...
        return TAGData(edge_index=edge_index, node_map=node_map, y=y, label_map=label_map, target_index=index,
                       edge_map=edge_map, ppr=self.ppr_scores)
...
class TextBase():
    ...
    def __text_to_embedding__(
        ...
        """
        embedding_file = osp.join(self.root, encoder_name, f"{name}_embedding.pt")

        # Check if the embedding exists and load if possible
        if from_saved and osp.exists(embedding_file):
            try:
                print("Load text embedding from saved file...")
                return torch.load(embedding_file)
            except Exception as e:
                warnings.warn(f"Failed to load cached embedding, regenerating: {str(e)}")

        # Determine if the feature should be saved
        file = None if name in ["question_features", "answer_features"] else osp.join(self.root, encoder_name,
                                                                                      f"{name}.pt")
